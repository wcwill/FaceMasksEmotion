# faceMask hDDM regression output plotting script: re-plot drift rate of false positives (IDing incorrect emotion) -- rating surprised
...
setwd("")
dra <- read.csv('faceMask2_hddm_drift_maskFalsePositives_ra_5000.csv', stringsAsFactors = FALSE)
rownames(dra) <- dra[,1]
...
## plot surprised drift (disgusted faces) by mask
setwd("")
drd <- read.csv('faceMask2_hddm_drift_maskFalsePositives_rd_5000.csv', stringsAsFactors = FALSE)
rownames(drd) <- drd[,1]
...
setwd("")
drf <- read.csv('faceMask2_hddm_drift_maskFalsePositives_rf_5000.csv', stringsAsFactors = FALSE)
rownames(drf) <- drf[,1]
...
setwd("")
drs <- read.csv('faceMask2_hddm_drift_maskFalsePositives_rs_5000.csv', stringsAsFactors = FALSE)
rownames(drs) <- drs[,1]
