...
setwd("")
dfa <- read.csv('faceMask_hddm_drift_maskFalsePositives_fa_5000.csv', stringsAsFactors = FALSE)
rownames(dfa) <- dfa[,1]
...
## plot fearful drift (happy faces) by mask
setwd("")
dfh <- read.csv('faceMask_hddm_drift_maskFalsePositives_fh_5000.csv', stringsAsFactors = FALSE)
rownames(dfh) <- dfh[,1]

# generate summary dataframe
mask <- c('lower', 'none', 'upper')
value <- c(dfh["v_C(mask)[T.lower]","mean"], dfh["v_Intercept","mean"], dfh["v_C(mask)[T.upper]","mean"])
cimin <- c(dfh["v_C(mask)[T.lower]","X2.5q"], dfh["v_Intercept","X2.5q"], dfh["v_C(mask)[T.upper]","X2.5q"])
cimax <- c(dfh["v_C(mask)[T.lower]","X97.5q"], dfh["v_Intercept","X97.5q"], dfh["v_C(mask)[T.upper]","X97.5q"])
dfh_drift_mask <- data.frame(mask, value, cimin, cimax)

# add baseline mask condition (i.e. no mask) intercept value to comparison mask conditions (i.e. lower/upper) to calculate non-relative coefficients and CIs
dfh_drift_mask$value[dfh_drift_mask$mask != 'none'] <- (dfh_drift_mask$value[dfh_drift_mask$mask != 'none'] + dfh_drift_mask$value[dfh_drift_mask$mask == 'none'])
dfh_drift_mask$cimin[dfh_drift_mask$mask != 'none'] <- (dfh_drift_mask$cimin[dfh_drift_mask$mask != 'none'] + dfh_drift_mask$value[dfh_drift_mask$mask == 'none'])
dfh_drift_mask$cimax[dfh_drift_mask$mask != 'none'] <- (dfh_drift_mask$cimax[dfh_drift_mask$mask != 'none'] + dfh_drift_mask$value[dfh_drift_mask$mask == 'none'])

# flip coefficients
dfh_drift_mask$value <- dfh_drift_mask$value * -1
dfh_drift_mask$cimin <- dfh_drift_mask$cimin * -1
dfh_drift_mask$cimax <- dfh_drift_mask$cimax * -1

# plot
my_palette <- brewer.pal(3, "Greens")[c(2,3,2)]
ggplot(dfh_drift_mask, aes(x=mask, y=value, fill = mask)) + 
  geom_bar(color = "black", stat = "identity") +
  geom_errorbar(aes(ymin=cimin, ymax=cimax), width=.2) +
  scale_fill_manual(values=c(my_palette[1], my_palette[2], my_palette[3])) +
...
setwd("")
dfs <- read.csv('faceMask_hddm_drift_maskFalsePositives_fs_5000.csv', stringsAsFactors = FALSE)
rownames(dfs) <- dfs[,1]
...
setwd("")
dfr <- read.csv('faceMask_hddm_drift_maskFalsePositives_fr_5000.csv', stringsAsFactors = FALSE)
rownames(dfr) <- dfr[,1]
